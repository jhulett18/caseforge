...
export async function generateReport(caseData, evidence, narratives) {
  ...
  y = drawInfoRow("Client:", caseData.client, MARGIN, y);
  drawInfoRow("Case Type:", caseData.caseType, col2, y + 16);
  y = drawInfoRow("Assigned PI:", caseData.assignedPI, MARGIN, y);
  drawInfoRow("Date Opened:", formatDate(caseData.openedDate), col2, y + 16);
  y = drawInfoRow("Status:", caseData.status, MARGIN, y);
  y -= 8;

  // ── SUBJECT PROFILE ──
  y = drawSectionTitle("SUBJECT PROFILE", y);
  y = drawInfoRow("Name:", caseData.subject.name, MARGIN, y);
  drawInfoRow("DOB:", formatDate(caseData.subject.dob), col2, y + 16);
